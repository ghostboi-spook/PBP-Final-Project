...
const markWatchedBtn = document.querySelector('button.bg-neutral-800.text-neutral-300');
if (markWatchedBtn) {
  markWatchedBtn.addEventListener('click', () => {
    ...
    if (!isWatched) {
      ...
      markWatchedBtn.classList.remove('bg-neutral-800', 'text-neutral-300');
...
if (reviewForm) {
  reviewForm.addEventListener('submit', (e) => {
    ...
    const reviewCountElement = document.querySelector('.text-sm.text-neutral-400');
    if (reviewCountElement && reviewCountElement.textContent.includes('User reviews')) {
      const currentText = reviewCountElement.textContent;
      const match = currentText.match(/User reviews · (\d+)/);
      if (match) {
        const currentCount = parseInt(match[1]);
        reviewCountElement.textContent = `User reviews · ${currentCount + 1}`;
      }
    }
  });
}
